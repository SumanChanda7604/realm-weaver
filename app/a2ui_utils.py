...
def a2ui_callback(
    ...
    for part in llm_response.content.parts:
        ...
        if not _surface_is_renderable(messages):
            # We recognized A2UI but couldn't recover a renderable surface.
            # Clean up raw A2UI JSON tags/fences and return the model's prose response!
            clean_text = _TAG_RE.sub("", text).strip()
            clean_text = re.sub(r'```(?:json)?[\s\S]*?```', '', clean_text).strip()
            clean_text = re.sub(r'\{\s*"beginRendering"[\s\S]*\}', '', clean_text).strip()
            if not clean_text or len(clean_text) < 15:
                clean_text = (
                    "🗺️ As you journey across the Shattered Peaks, dark clouds gather over the mountain rifts. "
                    "Wild elemental beasts lurk nearby. What action will you take next?"
                )
            return LlmResponse(
                content=types.Content(
                    role="model", parts=[types.Part(text=clean_text)]
                )
            )
